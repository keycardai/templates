...
|---|---|---|
| `KEYCARD_ZONE_URL` | — | Your zone, e.g. `https://<zone-id>.keycard.cloud` |
...
| `KEYCARD_CLIENT_ID` | — | The agent application's credential identifier |
| `KEYCARD_CLIENT_SECRET` | — | Its secret. Replace with workload identity when deploying. |
| `KEYCARD_AUTHORIZATION_PAGE` | `http://localhost:8765/` | Where interrupts send the user to sign in or consent |
| `KEYCARD_SUBJECT_TOKEN` | — | Written by `signin.py`. The signed-in user for runs that carry no per-run identity. |
...
2. **Per-caller identity.** `KEYCARD_SUBJECT_TOKEN` is a convenience for local
   ...
   `context=KeycardIdentity(subject_token=...)`, so two users produce two
   distinct delegation chains.
3. **Model access.** Keycard can broker Anthropic through Workload Identity
   Federation, which removes `ANTHROPIC_API_KEY` as well. See
