# SPEC — mcp-server-ruby

> A Keycard-protected MCP server with a single `hello` tool — the simplest way to see the full OAuth handshake end-to-end in Ruby and use it as a starting point for your own tools.

This template is a plain Rack MCP server. It runs as-is, but it has **no** authorization until Keycard is provisioned with the primitives below. An agent (e.g. the `keycard-template-app` skill) MUST perform the steps in this document before declaring the project ready.
...
A working MCP server built on plain Rack and the official MCP Ruby SDK (the [`mcp` gem](https://github.com/modelcontextprotocol/ruby-sdk)), protected by Keycard's OAuth 2.1 flow via `keycardai-mcp`. It ships one tool — `hello` — that takes a name and returns a greeting. The point isn't the tool; it's the protocol plumbing around it: OAuth metadata endpoints (`.well-known/oauth-protected-resource`, `.well-known/oauth-authorization-server`), bearer-token middleware that validates Keycard-minted JWTs, and a scope-gated `/mcp` endpoint. Once scaffolded, you extend it by registering more tools in `config.ru`.

`keycardai-mcp` wraps no MCP SDK. It attaches at the Rack seam, so the official `mcp` gem mounts behind it untouched and can be swapped for any other Rack-compatible handler.

Pick this template when you want to build a Ruby MCP server from scratch and need a working, minimal starting point that already handles authentication.

## 0b. One-line summary (for the template picker)

Minimal Keycard-protected MCP server (Ruby + Rack) with a `hello` tool — add your own tools and go.
...
Write `.env` in the project root. The template reads `KEYCARD_URL`, `KEYCARD_RESOURCE_ID` and `PORT` from the environment; load `.env` via your shell, `keycard run`, or a tool like `direnv`:
...
`KEYCARD_RESOURCE_ID` MUST match the Resource `identifier` created in §1c exactly. When set, the verifier binds the audience, so a token minted for a different resource is rejected. Leave it unset only if you have not provisioned a Resource yet.
...
The template ships no `Gemfile.lock`, so `bundle install` resolves against the user's Ruby. The agent MUST run `bundle install` before the smoke test, and SHOULD leave the generated `Gemfile.lock` in place for the user to commit.
...
```bash
bundle install
KEYCARD_URL="$KEYCARD_URL" PORT=<port> bundle exec rackup --port <port> &
SERVER_PID=$!
...
`scripts/ci-verify.sh` runs exactly these checks plus one more: an unauthenticated `POST /mcp` MUST return 401 with a `WWW-Authenticate` header carrying `resource_metadata`. That header is how an MCP client discovers where to authenticate, so a 401 without it means the server is unusable even though every other endpoint looks healthy.
...
- Do not add a `keycardai-oauth` line to the `Gemfile`. `keycardai-mcp` depends on it, and pinning it separately invites a resolution conflict.
...
| `build_command` | `bundle install` | this session | Installs the gems |
| `start_command` | `cd <name> && bundle exec rackup --port <port>` | their terminal (separate, keep open) | The server must be reachable at `http://localhost:<port>/mcp` for the remaining steps |
...
|---|---|
| Step 4 (copy) | "Copying the `mcp-server-ruby` blueprint into `./<name>` — a working Rack MCP server with a `hello` tool already wired up." |
| Step 6 (fill in) | "Installing the gems, filling in `.env` with your zone's OIDC issuer URL and a free local port, and dropping a `keycard.toml` next to it." |
...
- **Add a tool.** Call `mcp_server.define_tool(name:, description:, input_schema:)` with a block returning an `MCP::Tool::Response` in `config.ru`, alongside `hello`. Tools are gated by the `mcp:tools` scope the bearer middleware enforces.
- **Change the server.** `config.ru` is the whole Rack app. The metadata endpoints (`Keycardai::MCP::MetadataApp`) and bearer middleware (`Keycardai::MCP::RequireBearerAuth`) come from `keycardai-mcp`; you only touch those to change scopes or add a second resource.
- **Swap the MCP layer.** `keycardai-mcp` wraps no MCP SDK, so anything Rack-compatible can sit behind `RequireBearerAuth` in place of the `mcp` gem.
- **Read the caller's identity.** Inside the protected handler, `Keycardai::MCP.auth_info(env)` returns the verified token (`subject`, `scopes`, `claims`).
- **Add downstream credentials.** If a tool needs to call a third-party API, use `keycard-discover-entities` to find the Keycard entity URI and `keycard-upsert-config` to add a credential entry, brokered into the process by `keycard run`. `Keycardai::MCP::AuthProvider#grant` then exchanges the caller's token for one per downstream resource.
...
3. **Every request to `/mcp` returns 401 even with a fresh token.** `KEYCARD_RESOURCE_ID` does not match the Resource identifier the token was minted for, so the audience check rejects it. Both must be the exact `/mcp` URL from §1c.

4. **Port already in use at smoke-test time.** Re-probe the port, pick a new one, rewrite `.env`, retry.

5. **`keycard-vault` provider selected instead of `keycard-sts`.** Vault providers cannot mint OAuth tokens for an MCP Resource; token requests fail with `invalid_target`. Point the Resource's `credential_provider_id` at the zone's `keycard-sts` provider.

6. **Claude does not see the new MCP server.** Claude discovers MCP servers at startup and does not hot-reload. The user MUST restart Claude (`keycard run -- claude`) after `claude mcp add`.

7. **`bundle exec rackup` fails with "command not found".** Rack 3 moved `rackup` into its own gem. It is already in the `Gemfile`; the cause is a skipped `bundle install`.

8. **Ruby version too old.** `keycardai-mcp` requires Ruby >= 3.2. `bundle install` fails at resolution rather than at runtime.
